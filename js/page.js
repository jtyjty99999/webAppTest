...
var testData = {
	userName : localStorage.getItem('mirrtalkNumber'),
	agent : config.agent,
...
renderFromData('详细资料', {
	...
	info = {
		mirrtalkNumber : localStorage.getItem('mirrtalkNumber'),
		agent : config.agent,
		secret : config.secret,
		serviceType : "326baf"
	}
...
window.onload = function () {
	...
	if (!localStorage.getItem('mirrtalkNumber')) {
...
$doc.delegate('.edit', activate, function () {
	var _this = $(this);
	var flag = _this.attr('editing'),
	type = _this.attr('editType') || 'text',
	param = _this.attr('param');
